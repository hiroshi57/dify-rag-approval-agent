...
# 直前に算用数字や漢数字が無い、純粋な漢数字表記のみを対象にする。
# (これが無いと「1万5000円」の "万円" 部分にマッチして 10,000 円を偽の候補にしてしまう)
_KANJI_AMOUNT_RE = re.compile(
    r"(?<![0-9〇零一二三四五六七八九十百千万億])"
    r"[〇零一二三四五六七八九十百千][〇零一二三四五六七八九十百千万億]*\s*円")
